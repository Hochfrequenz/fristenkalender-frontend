...
| Backend image | `ghcr.io/hochfrequenz/fristenkalender-functions:2.1.0` | Public registry, pinned version |
...
const IMAGE = 'ghcr.io/hochfrequenz/fristenkalender-functions:2.1.0';
...
test('calendar displays deadlines from backend', async ({ page, backendUrl }) => {
